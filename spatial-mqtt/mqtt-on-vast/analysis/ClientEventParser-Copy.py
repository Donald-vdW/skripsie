...
def find_subscription_by_subid(subid_to_find):
    return spatial_subs.get(subid_to_find, None)


def find_unsubscription_by_subid(unsubid_to_find):
    return spatial_unsubs.get(unsubid_to_find, None)


def spatial_unsubscribe_by_subid(subid_to_delete):
    # Check if the subID exists in the dictionary
    if subid_to_delete in spatial_subs:
        # Move the subscription from spatial_subs to old_spatial_subs
        old_spatial_subs[subid_to_delete] = spatial_subs[subid_to_delete]
        # Remove the subscription from spatial_subs
    else:
...
def find_spatial_publication_by_pubid(pubid_to_find):
    return sent_spatial_pubs.get(pubid_to_find, None)
...
for event in events:
    ...
    if event['event'] == CLIENT_JOINS:  # Client joins
        event_without_clientID = {key: value for key, value in event.items() if key != 'id'}
        active_clients[event['id']] = event_without_clientID

        # Check if the broker exists in the set and add it if not
        if event['matcher'] not in brokers:
            brokers.add(event['matcher'])

    elif event['event'] == CLIENT_LEAVES:  # Client leaves
        left_clients[event['id']] = active_clients[event['id']]
        active_clients.pop(event['id'], None)
        ...
        # Create a copy of the event dictionary excluding 'subID'
        event_without_subID = {key: value for key, value in event.items() if key != 'subID'}
        # Use the 'subID' from the event as the key in spatial_unsubs
        spatial_unsubs[event['subID']] = event_without_subID
...
for received_pub in received_spatial_pubs:
    ...
    if matching_sent_pub is None:
        ...
        subID = received_pub['pub']['subID']
        sender = spatial_subs[subID]['clientID']
...
if spatial_subs:  # This checks if the dictionary is not empty
    for subID, sub in spatial_subs.items():
        print('\t\t\t', subID, ' ', sub['aoi'])
