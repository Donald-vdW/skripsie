...
const _alias = `C2` //DO NOT CHANGE THIS
...
client.on(`connect`, 
    function(packet) {
        ...
        spSubscribe({x: 100, y: 100, radius: 50, channel: `channel1`})
    }
)
...
client.on(`message`, 
    async function(topic, message, packet){
        ...
        if(message.toString() === `End Test` && topic === `Test`){
            doTest = false
            spUnsub({x: 100, y: 100, radius: 50, channel: `channel1`})
            console.log(`Ending...`)
