...
function doSubscribe (sub, done) {
  ...
  if (isSpatial(topic)) {
    const msg = extractSpatialData(topic)
    //console.log('Spatial subscribe <'+msg.x+','+msg.y+','+msg.radius+','+msg.channel+'>')
    var subscription = this.client.broker.matcher.subscribe(this.client.id, msg.x, msg.y, msg.radius, msg.channel, msg.subID);
    ...
    /*if (packet.messageId !== undefined) {
      // [MQTT-3.9.3-1]
      write(client,
        new SubAck(packet, this.subState.map(obj => obj.granted)),  noop)
    }*/
    done()
...
//  console.log('authorize')
 // console.log('authorize topic: <'+sub.topic+'>')
...
function storeSubscriptions (sub, done) {
  //console.log('storeSubScriptions')
...
function addSubs (sub, done) {
  ...
  //console.log('Subscribe.js::addSubs => deliverFunc assigned?')
  ...
  if (!rap) {
    ...
    func = function handlePacketSubscription (_packet, cb) {
      ...
      //console.log('Subscribe.js::addSubs => deliverFunc called')
